...
#define WEAK_BUNDLE_IDENTIFIER extern __attribute__((weak_import))
...
WEAK_BUNDLE_IDENTIFIER float LJUSB_GetLibraryVersion(void);
//Returns the labjackusb library version number.

WEAK_BUNDLE_IDENTIFIER unsigned int LJUSB_GetDevCount(unsigned long ProductID);
...
WEAK_BUNDLE_IDENTIFIER unsigned int LJUSB_GetDevCounts(UINT *productCounts, UINT * productIds, UINT n);
...
WEAK_BUNDLE_IDENTIFIER int LJUSB_OpenAllDevices(HANDLE* devHandles, UINT* productIds, UINT maxDevices);
...
WEAK_BUNDLE_IDENTIFIER HANDLE LJUSB_OpenDevice(UINT DevNum, unsigned int dwReserved, unsigned long ProductID);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_Write(HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_Read(HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_Stream(HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_WriteTO(HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_ReadTO(HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_StreamTO(HANDLE hDevice, BYTE *pBuff, unsigned long count, unsigned int timeout);
...
WEAK_BUNDLE_IDENTIFIER void LJUSB_CloseDevice(HANDLE hDevice);
// Closes the handle of a LabJack USB device.

WEAK_BUNDLE_IDENTIFIER bool LJUSB_IsHandleValid(HANDLE hDevice);
// Returns true if the handle is valid; this is, it is still connected to a
// device on the system.

WEAK_BUNDLE_IDENTIFIER unsigned short LJUSB_GetDeviceDescriptorReleaseNumber(HANDLE hDevice);
// Returns the device's release number (binary-coded decimal) stored in the
// device descriptor.
// hDevice = The handle for your device.

WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_GetHIDReportDescriptor(HANDLE hDevice, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_BulkRead(HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER unsigned long LJUSB_BulkWrite(HANDLE hDevice, unsigned char endpoint, BYTE *pBuff, unsigned long count);
...
WEAK_BUNDLE_IDENTIFIER bool LJUSB_AbortPipe(HANDLE hDevice, unsigned long Pipe);
